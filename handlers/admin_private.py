...
from ORM.ORM_Query import orm_get_customers, orm_add_product, orm_update_product, orm_delete_product, orm_get_products, \
    orm_get_product, orm_add_master, orm_get_master, orm_get_masters, orm_update_master, orm_delete_master
...
async def My_Products(message:types.Message,session:async_session):
    for product in await orm_get_products(session):
        await message.answer(f"{product.name}\n{product.description}\nPrice:{round(product.price,2)}$",
            ...
            btns={
                "Удалить":f"DeleteProduct_{product.id}",
                "Изменить":f"ChangeProduct_{product.id}"
                  }
            )
                )


@admin_router.callback_query(F.data.startswith("DeleteProduct_"))
...
@admin_router.callback_query(StateFilter(None),F.data.startswith("ChangeProduct_"))
...
@admin_router.message(F.text.lower() == "мастеры")
async def My_Masters(message: types.Message, session: async_session):

    for master in await orm_get_masters(session):
        await message.answer(f"{master.name}\n{master.description}\n",
                             reply_markup=inline_kb_builder(
                                 btns={
                                     "Удалить": f"DeleteMaster_{master.id}",
                                     "Изменить": f"ChangeMaster_{master.id}"
                                 }
                             )
                             )
...
@admin_router.callback_query(F.data.startswith("DeleteMaster_"))
async def delete_product(callback:types.CallbackQuery, session:async_session,):
    master_id = callback.data.split("_")[-1]
    await orm_delete_master(session, int(master_id))
    await callback.answer("Мастер удален",show_alert=True)
    await callback.message.delete()



@admin_router.callback_query(StateFilter(None),F.data.startswith("ChangeMaster_"))
async def change_product(callback:types.CallbackQuery, state:FSMContext, session:async_session,):
    master_id = callback.data.split("_")[-1]

    master_for_change = await orm_get_master(session, int(master_id))
    AddMaster.master_for_change = master_for_change

    await callback.answer()
    await callback.message.answer("Введите имя мастера", reply_markup=types.ReplyKeyboardRemove())
    await state.set_state(AddMaster.name)




@admin_router.message(StateFilter(None), F.text.lower() == "добавить мастера")
async def Addmaster(message:types.Message,state:FSMContext):
    await message.answer("Введите имя мастера",reply_markup=types.ReplyKeyboardRemove())
    await state.set_state(AddMaster.name)



@admin_router.message(AddMaster.name, or_f(F.text, F.text=="."))
async def Addmaster_name(message:types.Message,state:FSMContext):

    if message.text == ".":
        await state.update_data(name = AddMaster.master_for_change.name)
    else:
        await state.update_data(name = message.text)

    await message.answer("Введите описание продукта")
    await state.set_state(AddMaster.description)

@admin_router.message(AddMaster.name)
async def Addproduct_name(message:types.Message):
    await message.answer("Вы ввели некорректные данные\nПожалуйста, введите текстовое название товара")



@admin_router.message(AddMaster.description, or_f(F.text, F.text=="."))
async def Addmaster_description(message:types.Message,state:FSMContext, session:async_session):
    if message.text == ".":
        await state.update_data(description = AddMaster.master_for_change.description)
    else:
        await state.update_data(description = message.text)

    data = await state.get_data()
    try:
        if AddMaster.master_for_change:
            await orm_update_master(session, AddMaster.master_for_change.id, data)
            AddMaster.master_for_change = None
        else:
            await orm_add_master(session, data)

        await message.answer("Мастер добавлен", reply_markup=admin_kb)
        await state.clear()

    except Exception as e:
        await message.answer(f"Ошибка{e}.Спросите у вашего программиста. Он опять денег хочет...",
                             reply_markup=admin_kb)
        await state.clear()


@admin_router.message(AddProduct.description)
async def Addmaster_description(message:types.Message):
    await message.answer("Вы ввели некорректные данные\nПожалуйста, введите текстовое описание товара")
